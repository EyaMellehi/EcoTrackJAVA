...
public class FieldAgentsController {
    ...
    @FXML private TableView<User> tableFieldAgents;
    ...
    private ObservableList<User> allFieldAgents = FXCollections.observableArrayList();
    private User loggedUser;

    @FXML
    public void initialize() {
        colNum.setCellValueFactory(cellData ->
                new ReadOnlyObjectWrapper<>(tableFieldAgents.getItems().indexOf(cellData.getValue()) + 1)
        );
    ...
    private void loadFieldAgents() {
        try {
            List<User> users = userService.getFieldAgents();
            allFieldAgents.setAll(users);
            tableFieldAgents.setItems(allFieldAgents);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    void filterFieldAgents() {
        ...
        tableFieldAgents.setItems(FXCollections.observableArrayList(filtered));
    }

    private void deleteFieldAgent(User user) {
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Delete Field Agent");
    ...
    private void editFieldAgent(User user) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/admin/edit_field_agent.fxml"));
            Parent root = loader.load();

            EditFieldAgentController controller = loader.getController();
            controller.setUser(user);
            controller.setLoggedUser(loggedUser);

            Stage stage = (Stage) menuAdmin.getScene().getWindow();
            stage.setScene(new Scene(root));
            stage.setTitle("Edit Field Agent");
            stage.show();
        } catch (Exception e) {
            e.printStackTrace();
        }
