...
public class SubscribersController {
    ...
    @FXML private TableView<User> tableSubscribers;
    ...
    public void initialize() {
        colNum.setCellValueFactory(cellData ->
                new ReadOnlyObjectWrapper<>(tableSubscribers.getItems().indexOf(cellData.getValue()) + 1)
        );
    ...
    private void loadSubscribers() {
        try {
            List<User> users = userService.getCitoyens();
            allSubscribers.setAll(users);
            tableSubscribers.setItems(allSubscribers);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    void filterSubscribers() {
        ...
        List<User> filtered = allSubscribers.stream().filter(user -> {
        ...
        tableSubscribers.setItems(FXCollections.observableArrayList(filtered));
    }
    ...
    private void editSubscriber(User user) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/admin/edit_subscriber.fxml"));
            Parent root = loader.load();

            EditSubscriberController controller = loader.getController();
    ...
    private void deleteSubscriber(User user) {
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Delete Subscriber");
